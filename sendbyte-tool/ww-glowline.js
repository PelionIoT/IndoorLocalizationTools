...
<pre>
{
	...
	*/
	...
	enableGlowline : function(Mode,ONOFF,DLY,R,G,B,FLASH,R2,G2,B2) {
		// NOTE: all function must return an object in this format:
		var ret = {
			totalbytes: 0, // the amount of bytes to send
			buffer: null  // A Buffer object
	...
	enableMotion : function(ONOFF,SENS) {
		// NOTE: all function must return an object in this format:
		var ret = {
			totalbytes: 0, // the amount of bytes to send
			buffer: null  // A Buffer object
		...
		if(typeof SENS === undefined) {
			dlog("SENS not set. default 0x00");
			SENS = 0x00;
	    	}
	    	if(SENS < 0 || SENS > 0xff){
			errlog("SENS invalid. 0 - 255. Setting 0");
			SENS = 0x00;
	    	}
		...
		// fill up the buffer with what you need
		ret.buffer.writeUInt8(0x1,0); // put 0x0A in position 0
		ret.buffer.writeUInt8(parseInt(ONOFF),1); // put 0x0A in position 0
		ret.buffer.writeUInt8(parseInt(SENS),2);
